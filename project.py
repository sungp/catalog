...
def getItems(category_name):
    try:
        return session.query(Item).filter_by(category_name=category_name).all()
    except:
        return []
...
def catalogJSON():
    ...
    return jsonify(categories=[cat.do_serialize(getItems(cat.name))
    ...
    if request.method == 'POST':
        ...
        if not category_name:
            return ("<script>function myFunction() "
                    "{alert('You must specify category name');}"
                    "</script><body onload='myFunction()'>")

        item_title = request.form['title'].strip()
        if not item_title:
            return ("<script>function myFunction() "
                    "{alert('You must specify item name');}"
                    "</script><body onload='myFunction()'>")
...
def editItem(category_name, item_title):
    ...
    if request.method == 'POST':
        ...
        if request.form['category_name'] and \
